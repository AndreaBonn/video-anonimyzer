...
def find_chessboard_corners(image_paths, board_size=(9, 6)):
    ...
    for path in image_paths:
        ...
        if ret:
            ...
            print(f"  SKIP: scacchiera non trovata in {os.path.basename(path)}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Calibrazione camera per correzione fish-eye")
    parser.add_argument("--images", required=True,
                        help="Cartella con immagini di calibrazione")
    parser.add_argument("--output", default="calibration.npz",
                        help="File di output (.npz)")
    parser.add_argument("--board-cols", type=int, default=9,
                        help="Colonne interne scacchiera (default: 9)")
    parser.add_argument("--board-rows", type=int, default=6,
                        help="Righe interne scacchiera (default: 6)")
    ...
    obj_points, img_points, image_size = find_chessboard_corners(
        image_paths, board_size)

    if len(obj_points) < 3:
        print(f"\nErrore: servono almeno 3 immagini valide, "
              f"trovate solo {len(obj_points)}.")
        sys.exit(1)
